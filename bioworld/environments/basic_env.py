            ...
            for key in self.settings["agents"].keys():
                for i in range(self.settings["agents"][key]):
                    agent=self.agent_template(
                        self.settings, name='gen['+i+']-org['+key+'-'+str(i)+']nn', neuralnet=self.settings["neuralet"], actionspace=2, mind_type=self.settings['mind_type'], behaviour=key)
                    self.organisms.append() 

    def get_org_dict(self):
        org_dict={}
        for key in self.settings["agents"]: 
            org_dict[key] =[org for org in self.organisms if org.behaviour == key]
        return org_dict
    def prepare(self):
        self.populateEnv()
    ...
    def update_display(self, t_step):
        ...
        def compute_xy_values():
            ...
            range_multiplierx = .980 * \
                self.settings['pygame_worldbox'][1][0]/arena_diffx
            range_multipliery = .980 * \
                self.settings['pygame_worldbox'][1][1]/arena_diffy
            return x_coeff, y_coeff, range_modifierx, range_modifiery, range_multiplierx, range_multipliery

        def draw_organisms(x_coeff, y_coeff, range_modifierx, range_modifiery, range_multiplierx, range_multipliery):
        ...
        def draw_food(x_coeff, y_coeff, range_modifierx, range_modifiery, range_multiplierx, range_multipliery):
            for food in self.foods:
                pygame.draw.rect(
                    self.surface,
        ...
        x_coeff, y_coeff, range_modifierx, range_modifiery, range_multiplierx, range_multipliery = compute_xy_values()
        draw_background()
        draw_info_box(info_box(self, t_step))
        draw_top_text_line()
        draw_organisms(x_coeff, y_coeff, range_modifierx,
                       range_modifiery, range_multiplierx, range_multipliery)
        draw_food(x_coeff, y_coeff, range_modifierx, range_modifiery,
                  range_multiplierx, range_multipliery)
        self.screen.update()
            ...
            if len(carnivores) > 0  and len(herbivores) > 0:
                for prey in herbivores + omnivores:
                    for carnivore in carnivores:
                        ...
                        carnivore.r_food = 0
                        if carnivore.digest_t_step >= t_step:
                            herbivore_dist = dist(
                                carnivore.x, carnivore.y, prey.x, prey.y)
                            # DETECT FOOD
                            if herbivore_dist <= 0.075:
                                # UPDATE FITNESS FUNCTION
